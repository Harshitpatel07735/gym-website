...
import {
    LayoutDashboard, Users, CreditCard, Calendar, Bell,
    Search, Menu, X, LogOut, Zap, ChevronRight, ClipboardList,
    Clock, Heart, BarChart3,
} from 'lucide-react'
    ...
    return (
        <div className="min-h-screen bg-[#0a0a0a] text-white flex selection:bg-primary selection:text-white">
            ...
            <aside className={cn(
                ...
                <div className="p-8 pb-12 flex items-center justify-between">
                    {!isSidebarCollapsed && (
                        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="flex items-center gap-3">
                            ...
                            <span className="text-2xl font-black italic tracking-tighter">YOGI<span className="text-primary">.</span></span>
                        </motion.div>
                    )}
                ...
                <nav className="flex-1 px-4 space-y-2 overflow-y-auto">
                    {menuItems.map((item) => {
                        ...
                        return (
                            <Link key={item.label} href={`/admin?view=${item.view}`}
                                className={cn(
                                    'flex items-center gap-4 px-5 py-4 rounded-2xl transition-all duration-300 group relative',
                                    isActive ? 'bg-primary text-white shadow-[0_10px_30px_rgba(255,45,45,0.3)]' : 'text-white/40 hover:text-white hover:bg-white/5'
                                )}
                            >
                                ...
                                {isActive && (
                                    <motion.div layoutId="activeIndicator" className="absolute left-0 w-1.5 h-6 bg-white rounded-r-full" />
                                )}
                            </Link>
                        )
                    })}
            ...
            <main className="flex-1 flex flex-col h-screen overflow-hidden relative">
                <header className="h-20 bg-[#050505]/50 backdrop-blur-xl border-b border-white/5 px-6 flex items-center justify-between shrink-0 z-40">
                    ...
                    <div className="flex items-center gap-6">
                        <div className="hidden sm:flex relative group">
                            ...
                            <input type="text" placeholder="Search clients, bills..."
                ...
                <div data-lenis-prevent className="flex-1 overflow-y-auto p-6 md:p-8 bg-background relative"
